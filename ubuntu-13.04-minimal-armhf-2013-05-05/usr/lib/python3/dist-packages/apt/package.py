...
class Record(Mapping):
    ...
    def __iter__(self):
        return iter(list(self._rec.keys()))

    def iteritems(self):
        """An iterator over the (key, value) items of the record."""
        for key in list(self._rec.keys()):
...
class VersionList(Sequence):
    ...
    def __repr__(self):
        return '<VersionList: %r>' % list(self.keys())
    ...
    def get_changelog(self, uri=None, cancel_lock=None):
        ...
        try:
            try:
                ...
                changelog_file = urllib.request.urlopen(uri)
                ...
                return res if isinstance(res, str) else res.decode("utf-8")
            except (IOError, http.client.BadStatusLine):
