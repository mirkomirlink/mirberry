...
#if defined(CONFIG_SPLASH_IS_IN_MMC)
#include <mmc.h>
#endif
...
static void *video_logo(void)
{
	...
	if (s != NULL) {
		...
		int n;
		ulong dev, initblock, cnt_blocks, blk_size;
		
		if((s = getenv("splashimage_mmc_dev")) != NULL)
			dev = simple_strtoul (s, NULL, 16);

		if((s = getenv("splashimage_mmc_init_block")) != NULL)
			initblock = simple_strtoul (s, NULL, 16);

		if((s = getenv("splashimage_mmc_blkcnt")) != NULL)
			cnt_blocks = simple_strtoul (s, NULL, 16);

		//if((s = getenv("splashimage_mmc_blksize")) != NULL)
			//	blk_size = simple_strtoul (s, NULL, 16);
		
		struct mmc *mmc = find_mmc_device(dev);

		if (!mmc){
			printf ("Error Reading MMC.");
		}

		mmc_init(mmc);
		n = mmc->block_dev.block_read(dev, initblock, cnt_blocks, (ulong *)addr);
#endif
